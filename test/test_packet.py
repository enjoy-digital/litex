...
def loopback_test(dw):
    ...
    assert dut.header_errors == 0
    assert dut.data_errors == 0
    assert dut.first_errors == 0
    assert dut.last_errors == 0


def packet_fifo_test(layout, packets, payload_depth, param_depth=None, buffered=False):
...
def dispatcher_hold_sel_test(one_hot):
...
def test_header_get_field_width_mismatch():
    header = Header(
...
def test_header_encode_decode_no_swap():
    header = Header(
        fields={
            "field_8b":  HeaderField(0, 0, 8),
            "field_16b": HeaderField(1, 0, 16),
    ...
    assert observations["encoded"] == 0x345612
    assert observations["decoded_8b"] == 0x12
    assert observations["decoded_16b"] == 0x3456


def test_header_encode_decode_split_fields_with_swap():
    header = Header(
        fields={
    ...
    run_simulation_case(encoder, encode_stimulus())
    run_simulation_case(decoder, decode_stimulus())
    assert observations["encoded"] == 0x22114433
    assert observations["decoded"] == 0x11223344
...
@pytest.mark.parametrize("dw", [8, 32, 64, 128], ids=["8bit", "32bit", "64bit", "128bit"])
def test_loopback(dw):
    loopback_test(dw=dw)


def test_packet_fifo_with_params():
    ...
    packet_fifo_test(layout, packets, payload_depth=8, param_depth=2, buffered=True)


def test_packet_fifo_without_params():
    ...
    packet_fifo_test(layout, packets, payload_depth=6, buffered=False)


def test_packet_fifo_stress():
    ...
    packet_fifo_test(layout, packets, payload_depth=9, param_depth=1, buffered=True)


def test_packet_fifo_param_rollover():
    layout = EndpointDescription(
        payload_layout=[("data", 8)],
        param_layout=[("tag", 8), ("kind", 2)],
    )
    ...
    packet_fifo_test(layout, packets, payload_depth=4, param_depth=0, buffered=False)


def test_packet_fifo_alternating_sizes_buffered():
    layout = EndpointDescription(
        payload_layout=[("data", 8)],
        param_layout=[("tag", 8), ("kind", 2)],
    )
    ...
    packet_fifo_test(layout, packets, payload_depth=5, param_depth=1, buffered=True)


def test_packetizer_depacketizer_single_byte_payload_with_error():
    packets = [
        {
...
def test_packetizer_holds_error_on_buffered_last_beat():
    class DUT(Module):
        def __init__(self):
    ...
    assert any(beat["last"] for beat in observed)
    assert all(beat["error"] == 1 for beat in observed)


def test_packetizer_depacketizer_multiword_payload_with_error():
    packets = [
        {
            "field_8b": 0x12, "field_16b": 0x3456, "field_32b": 0x789abcde,
            "field_64b": 0x0123456789abcdef, "field_128b": 0x112233445566778899aabbccddeeff00,
...
def test_packetizer_sets_first_on_header():
    class DUT(Module):
        def __init__(self):
    ...
    assert all(first == 0 for first, _ in observed[1:])


def test_arbiter_packet_lock():
    layout = EndpointDescription(payload_layout=[("data", 8)], param_layout=[("tag", 4)])
...
def test_arbiter_multi_packet_alternation():
    layout = EndpointDescription(payload_layout=[("data", 8)], param_layout=[("tag", 4)])
    ...
    assert received_tags == [0, 1, 0, 1]


@pytest.mark.parametrize("one_hot", [False, True], ids=["binary", "one_hot"])
def test_dispatcher_hold_sel(one_hot):
    dispatcher_hold_sel_test(one_hot=one_hot)


def test_dispatcher_holds_sel_before_first_handshake():
    layout = EndpointDescription(payload_layout=[("data", 8)], param_layout=[("tag", 4)])
...
def test_dispatcher_invalid_sel_drops_packet():
    layout = EndpointDescription(payload_layout=[("data", 8)], param_layout=[("tag", 4)])
