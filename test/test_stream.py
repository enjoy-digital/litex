...
class TestStream(unittest.TestCase):
    ...
    def test_crossbar(self):
        dut = Crossbar(EndpointDescription(
            payload_layout=[("data", 8)],
            param_layout=[("tag", 4)],
        ), n=2)
        received0 = []
        ...
        def generator():
            yield dut.mux.sel.eq(0)
            yield dut.demux.sel.eq(1)
            yield dut.sink0.valid.eq(1)
            yield dut.sink0.first.eq(1)
            yield dut.sink0.last.eq(1)
            yield dut.sink0.data.eq(0x5a)
            yield dut.sink0.tag.eq(6)
            yield
            while (yield dut.sink0.ready) == 0:
                yield
            yield dut.sink0.valid.eq(0)
            ...
            yield dut.mux.sel.eq(1)
            yield dut.demux.sel.eq(0)
            yield dut.sink1.valid.eq(1)
            yield dut.sink1.first.eq(1)
            yield dut.sink1.last.eq(1)
            yield dut.sink1.data.eq(0xa5)
            yield dut.sink1.tag.eq(9)
            yield
            while (yield dut.sink1.ready) == 0:
                yield
            yield dut.sink1.valid.eq(0)
        ...
        def checker():
            ...
            yield dut.source1.ready.eq(1)
            for _ in range(6):
                if (yield dut.source0.valid):
                    received0.append((
                        (yield dut.source0.data),
                        (yield dut.source0.tag),
                        (yield dut.source0.first),
                        (yield dut.source0.last),
                    ))
                if (yield dut.source1.valid):
                    received1.append((
                        (yield dut.source1.data),
                        (yield dut.source1.tag),
                        (yield dut.source1.first),
                        (yield dut.source1.last),
                    ))
                yield

        run_simulation(dut, [generator(), checker()])
        self.assertEqual(received0, [(0xa5, 9, 1, 1)])
        self.assertEqual(received1, [(0x5a, 6, 1, 1)])

    def test_crossbar_invalid_selection_blocks_transfer(self):
        dut = Crossbar(EndpointDescription(
            payload_layout=[("data", 8)],
            param_layout=[("tag", 4)],
        ), n=3)
        ...
        def stimulus():
            ...
            yield dut.source1.ready.eq(1)
            yield dut.source2.ready.eq(1)
            yield dut.mux.sel.eq(3)
            yield dut.demux.sel.eq(0)
            yield dut.sink0.valid.eq(1)
            yield dut.sink0.first.eq(1)
            yield dut.sink0.last.eq(1)
            yield dut.sink0.data.eq(0x33)
            yield dut.sink0.tag.eq(3)
            yield
            observations["sink0_ready"] = (yield dut.sink0.ready)
            observations["source0_valid"] = (yield dut.source0.valid)
            observations["source1_valid"] = (yield dut.source1.valid)
