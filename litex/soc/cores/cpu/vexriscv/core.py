...
class VexRiscv(CPU, AutoCSR):
    ...
    def __init__(self, platform, variant="standard", with_timer=False):
        ...
        # Add CLIC support if the variant includes it
        # Check if this is a variant with CLIC (e.g., generated from GenFull with CLIC)
        if variant == "full":  # Assuming full variant now has CLIC support
