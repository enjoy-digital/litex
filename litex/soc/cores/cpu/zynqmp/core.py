...
class ZynqMP(CPU):
    ...
    def add_mio_config(self, directions,
        iotype   = "",
        slew     = "",
        pullup   = "disable",
        strength = None,
        ...
        assert polarity is None or isinstance(polarity, (dict, str))
        for i, direction in directions.items():
            ...
            mio_strength = strength.get(i, None) if strength is not None       else None
            mio_polarity = polarity.get(i, None) if isinstance(polarity, dict) else polarity
            assert mio_iotype   in ["", "cmos", "schmitt"]
            assert mio_strength is None or isinstance(mio_strength, int)
            assert mio_polarity is None or isinstance(mio_polarity, str)
            config = {
            ...
            if mio_strength is not None:
                assert mio_strength in [2, 4, 8, 12]
                config[f"PSU_MIO_{i}_DRIVE_STRENGTH"] = mio_strength
            if mio_polarity is not None:
                config[f"PSU_MIO_{i}_POLARITY"]       = mio_polarity
    ...
    with_ptp: bool
        Enable PTP support.
    iotype: str
        MIO input type (cmos/schmitt). Only used for RGMII RX and the MDIO
        signal in MIO mode.
    slew: str
        MIO slew rate (slow/fast/...). Only used for RGMII TX and the
        MDIO interface in MIO mode.
    strength: (optional) dict(pad(str), value(int))
        Tx Drive Strength. the RX side is fixed to 12mA.
        Pad/keys are clk, data, ctl, mdc, mdio.
    ...
    def add_ethernet(self, n=0,
        ...
        *,
        ...
        config = {
            ...
            f"PSU__ENET{n}__PTP__ENABLE"        : int(with_ptp),
        }

        # FIXME: TSU and FIFO are always disabled.
        config.update({
            "PSU__GEM__TSU__ENABLE"       : 0,
            f"PSU__ENET{n}__TSU__ENABLE"  : 0,
            "PSU__TSU__BUFG_PORT_PAIR"    : 0,
            f"PSU__ENET{n}__FIFO__ENABLE" : 0,
        })

        if mdio_pads is not None:
            config[f"PSU__ENET{n}__GRP_MDIO__IO"] = mdio_io_type
        ...
        if gt_lane is None and eth_io_type != "EMIO":
            ...
            drv_strength = {i: 4      if index < 6 else 12     for index, i in enumerate(eth_pins)}

            if strength is not None:
                drv_strength[eth_pins[0]] = strength.get("clk", 4)
                drv_strength[eth_pins[5]] = strength.get("ctl", 4)
                for i in range(1, 5):
                    drv_strength[eth_pins[i]] = strength.get("data", 4)
        ...
        if mdio_pads is not None and mdio_io_type != "EMIO":
            ...
            drv_strength = {
                mdio_pins[0]: 4 if strength is None else strength.get("mdc",  4),
                mdio_pins[1]: 4 if strength is None else strength.get("mdio", 4),
            }
            self.add_mio_config(
                directions = {mdio_pins[0]: "out",  mdio_pins[1]: "inout"},
    ...
    strength: (optional) dict(pad(str), value(int))
        Drive Strength.
        Pad/keys are sclk, ss2 ss1 ss0 miso mosi.
        Value range is [2, 4, 8, 12].
        When None default 4mA is used for both pads, if one of pad is missing default
        value is used too.
        This parameter is only used in MIO mode.
    ...
    freq: float (optional, keyword-only)
        PSU reference clock frequency in MHz. Zero keeps the default value.
    """
    def add_spi(self, n, pads_or_mio_group,
        ss1_en   = False,
        ss2_en   = False,
        iotype   = "cmos",
        slew     = "slow",
        ...
        *,
        ...
        config = {
            ...
            f"PSU__SPI{n}__GRP_SS0__ENABLE"    : 1,
            f"PSU__SPI{n}__GRP_SS0__IO"        : mio_ss0,
            f"PSU__SPI{n}__GRP_SS1__ENABLE"    : {True: 1, False: 0}[ss1_en],
            f"PSU__SPI{n}__GRP_SS2__ENABLE"    : {True: 1, False: 0}[ss2_en],

        }
        if ss1_en:
            config[f"PSU__SPI{n}__GRP_SS1__IO"] = mio_ss1
        if ss2_en:
            config[f"PSU__SPI{n}__GRP_SS2__IO"] = mio_ss2

        if srcsel:
            config[f"PSU__CRL_APB__SPI{n}_REF_CTRL__SRCSEL"]  = srcsel
        if freq:
            config[f"PSU__CRL_APB__SPI{n}_REF_CTRL__FREQMHZ"] = int(freq)
        ...
        if io_type != "EMIO":
            ...
            if strength is not None:
                pins_idx = dict(zip(pins_name, pins))
                for k,v in strength.items():
                    assert k in pins_name
                    assert isinstance(v, int)
                    drv_strength[pins_idx[k]] = v
            self.add_mio_config(
    ...
    iotype: str
        RX IO type for the MIO pins (cmos/schmitt). TX and MIOCLK (if enabled)
        are fixed. This parameter is only used in MIO mode.
    slew: str
        TX IO slew rate (slow/fast/...). RX and MIOCLK (if enabled) are fixed.
        This parameter is only used in MIO mode.
    tx_strength: (optional) int (default: 4)
        TX Driver Strength. Value range is [2, 4, 8, 12].
        When None default 4mA is used
        This parameter is only used in MIO mode.
    """
    def add_can(self, n, pads_or_mio_group,
        ext_clk     = None,
        ext_clk_freq= None,
        iotype      = "cmos",
        slew        = "fast",
        ...
        if ext_clk is not None:
            self.add_mio_config({ext_clk: "in"},
                iotype   = iotype,
                slew     = slew,
                pullup   = "pullup",
                strength = {ext_clk: 12},
