...
class ZynqMP(CPU):
    ...
    slew: str
        MIO slew rate (slow/fast/...).
    drive_strength: (optional) dict key/int
    ...
    freq: float (optional, keyword-only)
        PSU reference clock frequency in MHz. Zero keeps the default value.
    """
    def add_qspi(self, mode="Single", data_mode="x1", feedback_clk=False,
        iotype="cmos", slew="fast", drive_strength=None, *, srcsel="", freq=0):
        ...
        if drive_strength is not None:
            ...
            for k, v in drive_strength.items():
