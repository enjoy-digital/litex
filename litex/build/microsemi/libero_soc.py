...
class MicrosemiLiberoSoCToolchain(GenericToolchain):
    ...
    def finalize(self):
        ...
        if self.die.startswith("M2GL"):
            ...
            raise ValueError(f"unknown family for die: {self.die}")
    ...
    def build_timing_constraints(self, vns):
        ...
        for clk, [period, name] in sorted(self.clocks.items(), key=lambda x: x[0].duid):
            ...
            if name is None:
                name = clk_sig
            sdc.append(
                "create_clock -name {name} -period {period} [get_nets {clk}]".format(
                    name=name, period=period, clk=clk_sig))
        for from_, to in sorted(self.false_paths,
            ...
            sdc.append(
                ...
                "-asynchronous".format(from_=vns.get_name(from_), to=vns.get_name(to)))
