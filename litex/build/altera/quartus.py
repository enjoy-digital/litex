...
class AlteraQuartusToolchain(GenericToolchain):
    ...
    def __init__(self):
        ...
        self.cst                     = []
        self.qsys_systems            = {} # Platform Designer systems generated at build time (name: Tcl).
        self.ip_dir                  = _get_quartus_ip_dir(self._synth_tool)

    # Qsys Systems (Platform Designer) -------------------------------------------------------------

    def add_qsys_system(self, name, tcl):
        # Register a Platform Designer system to be generated from Tcl at build time (Pro flow).
        # Used by the Agilex HPS CPU to generate the HPS (+ optional HPS-EMIF) system.
        assert name not in self.qsys_systems
        self.qsys_systems[name] = tcl
    ...
    def build_project(self):
        ...
        # Add Qsys systems (generated from Tcl at build time, see build_script).
        for name, tcl in self.qsys_systems.items():
            tools.write_to_file(f"{name}.tcl", "\n".join(tcl))
            qsf.append(f"set_global_assignment -name QSYS_FILE {name}.qsys")
    ...
    def build_script(self):
        ...
        # IP Generation (Pro flow only): generate LiteX Platform Designer systems before synthesis.
        # The Standard flow (quartus_map) does not use Qsys systems and is left untouched; user IPs
        # added via platform.add_ip() are expected to be pre-generated (as done historically).
        # (qsys-script/qsys-generate are resolved when the script runs, like the other Quartus tools.)
        if (self._synth_tool == "quartus_syn") and len(self.qsys_systems):
            for name in self.qsys_systems.keys():
                # Tcl -> .qsys, then .qsys -> synthesis Verilog.
                script_contents += f"qsys-script --script={name}.tcl{fail_stmt}\n"
                script_contents += f"qsys-generate {name}.qsys --synthesis=VERILOG{fail_stmt}\n"
