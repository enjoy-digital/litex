...
class XilinxVivadoToolchain(GenericToolchain):
        ...
        if self._synth_mode == "vivado":
            ...
            for filename, language, library, *copy in self.platform.sources:
                ...
                if (language == "systemverilog"):
                    ...
                    tcl.append("if {[string equal [file extension " + filename_tcl + "] \".h\"]} {")
    ...
    def build_script(self):
        ...
        if self._synth_mode == "yosys":
            yosys_call = common._build_yosys_project(platform=self.platform, build_name=self._build_name)
            script_contents += yosys_call.rstrip("\n") + fail_stmt + "\n"

        script_contents += "vivado -mode batch -source " + self._build_name + ".tcl" + fail_stmt + "\n"
    ...
    def run_script(self, script):
        ...
        if which("vivado") is None and not os.getenv("LITEX_ENV_VIVADO", ""):
