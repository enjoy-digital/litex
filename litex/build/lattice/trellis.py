...
_build_template = [
    ...
    "ecppack {build_name}.config --svf {build_name}.svf --bit {build_name}.bit --bootaddr {bootaddr} {spimode} {idcode}"
]

def _build_script(source, build_template, build_name, architecture, package, speed_grade, timingstrict, ignoreloops, bootaddr, seed, spimode, idcode):
    ...
    for s in build_template:
        ...
        script_contents += s_fail.format(
            ...
            idcode          = "" if idcode is None else "--idcode {}".format(idcode))
...
class LatticeTrellisToolchain:
    ...
    def build(self, platform, fragment,
        ...
        **kwargs):
        ...
        # Nextpnr does not have option for 12f architecture, since it considers it's the same as 25f
        idcode = None
        if size == "12f":
            size = "25f"
            idcode = "0x21111043"
        ...
        script = _build_script(False, self.build_template, build_name, architecture, package,
                               speed_grade, timingstrict, ignoreloops, bootaddr, seed, spimode, idcode)
        # Run
        if run:
            _run_script(script)
