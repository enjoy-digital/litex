...
class InterfaceWriter:
    ...
    def generate_xml_blocks(self):
        ...
        for block in self.xml_blocks:
            if isinstance(block, InterfaceWriterXMLBlock):
                ...
                elif block["type"] == "TRION_DDR":
                    self.add_trion_ddr_xml(root, block)
    ...
    @staticmethod
    def add_trion_ddr_xml(root, block):
        # Trion DDR controllers are configured directly in the Efinity peripheral XML.
        ddr_info = root.find("efxpt:ddr_info", namespaces)
        if ddr_info is None:
            raise ValueError("Efinity peripheral XML has no DDR information section.")

        ddr = et.SubElement(ddr_info, "efxpt:ddr",
            name            = block["name"],
            ddr_def         = block["location"],
            cs_preset_id    = str(block["preset_id"]),
            cs_mem_type     = block["memory_type"],
            cs_ctrl_width   = "x{}".format(block["controller_width"]),
            cs_dram_width   = "x{}".format(block["dram_width"]),
            cs_dram_density = block["memory_density"],
            cs_speedbin     = str(block["speedbin"]),
            target0_enable  = "true",
            target1_enable  = str(block["port_count"] > 1).lower(),
            ctrl_type       = "none",
        )

        for n in range(block["port_count"]):
            target = et.SubElement(ddr, "efxpt:gen_pin_target{}".format(n))
            for signal, is_bus in _trion_ddr_axi_pins:
                et.SubElement(target, "efxpt:pin",
                    name      = "{}{}_{}".format(block["interface_name"], n, signal),
                    type_name = "{}_{}".format(signal.upper(), n),
                    is_bus    = str(is_bus).lower(),
                )
            et.SubElement(target, "efxpt:pin",
                name          = block["clock_name"],
                type_name     = "ACLK_{}".format(n),
                is_bus        = "false",
                is_clk        = "true",
                is_clk_invert = "false",
            )

        config = et.SubElement(ddr, "efxpt:gen_pin_config")
        for name in _trion_ddr_config_pins:
            et.SubElement(config, "efxpt:pin",
                name      = "",
                type_name = name,
                is_bus    = "false",
            )
        ...
        parameter_groups = [
            ("cs_fpga",         block["fpga_config"]),
            ("cs_memory",       block["memory_config"]),
            ("cs_memory_timing", block["memory_timing"]),
            ("cs_control",      block["control_config"]),
            ("cs_gate_delay",   block["gate_delay_config"]),
        ]
        for group_name, parameters in parameter_groups:
            group = et.SubElement(ddr, "efxpt:{}".format(group_name))
            for name, value in parameters.items():
                if isinstance(value, bool):
                    ...
                    value_type = "str"
                    value      = str(value)
                et.SubElement(group, "efxpt:param",
                    name       = name,
                    value      = value,
