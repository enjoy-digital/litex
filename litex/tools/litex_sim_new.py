...
def sim_args(parser):
    ...
    # UART.
    parser.add_argument("--uart-tcp",      action="store_true",
                        help="Use serial2tcp external module for UART.")
    parser.add_argument("--uart-tcp-port", type=int, default=1234,
                        help="TCP port for serial2tcp (default: 1234).")
    parser.add_argument("--uart-pty",      action="store_true",
                        help="Create a PTY bridged to the UART TCP port (requires socat).")
    parser.add_argument("--uart-pty-path", default="/tmp/litex_pty0",
                        help="Path for UART PTY (default: /tmp/litex_pty0).")
...
def main():
    ...
    if soc_kwargs["uart_name"] == "serial":
        ...
        if args.uart_tcp or args.uart_pty:
            ...
            # Console (stdin/stdout) UART bridge.
            sim_config.add_module("serial2console", "serial")
    else:
        if args.uart_tcp or args.uart_pty:
            print("[litex_sim] Warning: --uart-tcp/--uart-pty specified but uart_name is not 'serial'.")

    # If requested, create a PTY bridged to the TCP UART using socat.
    socat_proc = None
    if args.uart_pty:
        port     = args.uart_tcp_port
        ...
        cmd = [
            "socat",
            f"pty,link={pty_path},raw,echo=0",
            f"tcp:127.0.0.1:{port}",
        ]
        try:
            socat_proc = subprocess.Popen(cmd)
            print(f"[litex_sim] UART PTY created at: {pty_path}")
            print(f"[litex_sim] Connect with, for example:")
            print(f"  picocom -b 115200 {pty_path}")
